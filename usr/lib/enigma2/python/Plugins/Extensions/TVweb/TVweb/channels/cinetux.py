...
def findvideos(item):
    ...
    patron += '<td class="fuente-td[^>]+>([^<]+)</td>[^<]+'
    #patron += '<td class="link-td">(.*?)</td>'
    patron += '<td class="link-td"><span><a rel="nofollow" target="_blank" class="myButtonLink" href="([^"]+)"'
    matches = re.compile(patron,re.DOTALL).findall(data)
    scrapertools.printMatches(matches)
    for scrapedtitle,scrapedserver,scrapedlanguage,scrapedquality,scrapedlink in matches:
        title = "Ver "+scrapedtitle+" en "+scrapedserver+" ("+scrapedlanguage+") ("+scrapedquality+")"
        ...
        thumbnail = item.thumbnail
        plot = ""
        itemlist.append( Item(channel=__channel__, action="play", title=title , fulltitle=item.fulltitle+" ["+scrapedlanguage+"]["+scrapedquality+"]", url=url , thumbnail=thumbnail , plot=plot , fanart="http://pelisalacarta.mimediacenter.info/fanart/cinetux.jpg" , server = scrapedserver, folder=False) )
